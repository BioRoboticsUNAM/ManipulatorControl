...
{
    public class CmdLaRelPos : AsyncCommandExecuter
    {
        ...
        {
            TextBoxStreamWriter.DefaultLog.WriteLine("Cmd LaRelPos Received: " + command.StringToSend);
            ...
            if (command.HasParams)
            {
                ...
                {
                    TextBoxStreamWriter.DefaultLog.WriteLine("Cmd LaRelPos: Left Arm is busy executing another command");
                    return Response.CreateFromCommand(command, false);
                }
                if (!this.Signature.CallIfMatch(command))
                {
                    TextBoxStreamWriter.DefaultLog.WriteLine("Cmd LaRelPos: No match found with parameters options");
                    return Response.CreateFromCommand(command, false);
                }

                //this.CommandManager.Busy = true;

                succes = this.taskPlanner.LaGoToRelativeCartesianPos(this.x, this.y, this.z, this.roll, this.pitch, this.yaw, this.elbow);
            ...
            {
                TextBoxStreamWriter.DefaultLog.WriteLine("Cmd LaRelPos: Parameters required");
                return Response.CreateFromCommand(command, false);
            }
        }
